...
public class Gui
{
    ...
    public static void drawCenteredStringCustom(MinecraftFontRenderer normal, String text, int x, int y, int color)
    {
        normal.drawStringWithShadow(text, (float)(x - normal.getStringWidth(text) / 2), (float)y, color);
    }
    ...
    public static void drawModalRectWithCustomSizedTextureDouble(double x, double y, float u, float v, float width, float height, float textureWidth, float textureHeight) {
        float f = 1.0F / textureWidth;
        float f1 = 1.0F / textureHeight;
        Tessellator tessellator = Tessellator.getInstance();
        WorldRenderer worldrenderer = tessellator.getWorldRenderer();
    ...
    public static void drawScaledCustomSizeModalRect(float x, float y, float u, float v, int uWidth, int vHeight, float w, float h, float tileWidth, float tileHeight) {
      float f = 1.0F / tileWidth;
      float f1 = 1.0F / tileHeight;
      Tessellator tessellator = Tessellator.getInstance();
      WorldRenderer worldrenderer = tessellator.getWorldRenderer();
      worldrenderer.begin(7, DefaultVertexFormats.POSITION_TEX);
      worldrenderer.pos(x, (y + h), 0.0D).tex((u * f), ((v + vHeight) * f1)).endVertex();
      worldrenderer.pos((x + w), (y + h), 0.0D).tex(((u + uWidth) * f), ((v + vHeight) * f1)).endVertex();
      worldrenderer.pos((x + w), y, 0.0D).tex(((u + uWidth) * f), (v * f1)).endVertex();
      worldrenderer.pos(x, y, 0.0D).tex((u * f), (v * f1)).endVertex();
      tessellator.draw();
    }
